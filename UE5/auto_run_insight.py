    ...
    try:
        subprocess.check_call(
            [
                ...
                "start",
                "-n",
                package_name + "/com.epicgames.unreal.SplashActivity",
            ]
        )
    ...
    try:
        ...
        for file in os.listdir(path):
            if file.endswith(".utrace"):
                ...
                if modified_timestamp >= current_timestamp - datetime.timedelta(
                    seconds=10
                ):
                    return True
...
if __name__ == "__main__":
    if is_android_device_connected():
        # 执行两条指令
        execute_adb_command("adb reverse tcp:1981 tcp:1981")
        # execute_adb_command(
        #     "adb shell setprop debug.ue.commandline -tracehost=127.0.0.1"
        # )

        # 打开本地的UnrealInsights程序
        local_program_path = "C:\\Program Files\\Epic Games\\UE_5.4\\Engine\\Binaries\\Win64\\UnrealInsights.exe"
        open_local_program(local_program_path)

        # 打开Android设备上的名为“TRing”的app
        app_package_name = "com.Tencent.TRingDev"
        start_app(app_package_name)

        # 检查本地路径下是否生成了新的.utrace文件
        utrace_folder_path = "C:\\Users\\admin\\AppData\\Local\\UnrealEngine\\Common\\UnrealTrace\\Store\\001"
