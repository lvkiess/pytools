...
def search_and_display(search_text, dfs1, dfs2, search_column2, data_column2):
    ...
    # 获取第一个文件选定的工作表名称
    first_file_selected_sheet_name = sheet_select1[
        0
    ]  # 假设只选择了第一个文件的一个工作表
    ...
    if matched_rows.any():
        ...
        for sheet_name in sheet_select2:
            ...
            for index in matched_rows[matched_rows].index:
                ...
                try:
                    matched_result = df2[
                        df2.iloc[:, 0] == df1.at[index, df1.columns[0]]
                    ].iloc[0, 2]
                    found_matches.append(
                        (
                            first_file_selected_sheet_name,
                            search_text,
                            matched_result,
                            sheet_name,
                        )
    ...
    for sheet_name in sheet_select2:
        ...
        st.write(dfs2[sheet_name].iloc[:, [0, 2]])
